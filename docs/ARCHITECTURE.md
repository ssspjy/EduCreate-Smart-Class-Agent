...
> 比赛版已实现三类互动题。采用**后端固定 Jinja2 模板 + 结构化大纲要点**的策略，不执行用户或 LLM 返回的 HTML/JS。

### 5.8.1 支持类型与触发条件

| 类型 | 触发关键词示例 | 模板 | 输出 |
|---|---|---|---|
| 选择题互动 | 预览页选择“选择题” | 固定 Jinja2 题型模板 | HTML5 单页 quiz |
| 判断题互动 | 预览页选择“判断题” | 固定 Jinja2 题型模板 | HTML5 单页 quiz |
| 填空题互动 | 预览页选择“填空题” | 固定 Jinja2 题型模板 | HTML5 单页 quiz |
...
后端模板引擎负责把已校验的 bullets 渲染为 HTML；前端仅用 sandbox iframe 预览，无服务端代码生成风险。

### 5.8.3 边界情况

- **要点数量不足**：按实际要点数量生成并返回 warning，不伪造知识点
...
比赛版先保证 `.pptx` 和 `.docx` 两个评分相关主产物，同时提供独立 HTML5 互动预览。GIF / MP4 属于增强项，不计入当前完成能力。
...
| 互动内容 | `.html` | 固定 Jinja2 模板渲染，前端 sandbox 预览 | **已实现选择 / 判断 / 填空三类** |
...
- **HTML5 互动**：后端 Jinja2 固定模板 + 前端 sandbox iframe
